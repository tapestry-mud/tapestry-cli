...
describe('ensureContentGlobs', () => {
  it('adds all globs when the content block is absent', () => {
    ...
    expect(added.sort()).toEqual(['area_definitions', 'items', 'mobs', 'oracle_tables', 'places_oracle', 'rooms']);
  ...
  it('adds only missing globs when rooms is already present', () => {
    ...
    const added = ensureContentGlobs(tmp);
    expect(added.sort()).toEqual(['area_definitions', 'items', 'mobs', 'oracle_tables', 'places_oracle']);
    const m = readYaml(path.join(tmp, 'pack.yaml'));
